...
- New configs have been added in `configs/` and can be run the same way as detailed above.
- The raw CASIA dataset can be downloaded [here](https://nlpr.ia.ac.cn/databases/Download/Offline/TextlineData/HWDB2.1Test.zip) and pre-processed with `notebooks/00_casia_preprocessing.py`
- `Malvern-Hills-5+`/`10+` and `IAM-5`/`IAM-5-Random` are generated by the updated `notebooks/00_{malvern_hills,iam}_preprocessing.py`
- Experiments for TrOCR, PyLaia and DocOwl2 can be run with the notebooks in `notebooks/additional_models`
- Malvern-Hills ablations can be obtained with `notebooks/02_mhills_ablations.py`
- Cached MLLM results for the new experiments are included in the updated MLLM cache files [here](https://judgeocr.s3.eu-north-1.amazonaws.com/gpt_cache_dbs_rebuttals.zip) (replace the existing ones)
- The updated Malvern-Hills dataset with cropped lines and metadata can be downloaded [here](https://judgeocr.s3.eu-north-1.amazonaws.com/malvern_hills_trust_updated.zip)