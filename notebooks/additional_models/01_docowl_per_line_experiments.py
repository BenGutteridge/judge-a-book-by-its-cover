...
def main(dir_to_process: str):
    ...
    single_line_prompt = "Given the above handwritten line from a document page, transcribe the full text of the line accurately. Return all of the text in the line in reading order. Return only the transcribed text. Use correct capitalization."
    ...
    for root, dirs, files in sorted(os.walk("malvern_hills_trust")):
        if not root.startswith(dir_to_process):
            continue
        for file in sorted(files):
            if file.endswith(".png"):
                ...
                print(f"\nProcessing {img_path}...")
                t0 = time.time()
                res: str = docowl.inference([img_path], query=single_line_prompt)
                t1 = time.time() - t0
                print(f"Result:\n{res}")
                # Save to text file
